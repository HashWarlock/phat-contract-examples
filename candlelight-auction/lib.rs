// This is an example to show porting of an ink based smart contract w/ minor modifications for fat contracts.
// All development and credit goes to the following author below:
    ...
    impl CandleAuction {
        ...
        pub fn new(
            ...
            ink_lang::codegen::initialize_contract(|contract: &mut Self| {
                ...
                contract.opening_period = opening_period;
                contract.ending_period = ending_period;
                contract.winning = None;
                contract.winner = None;
                contract.finalized = false;
                contract.winning_data = winning_data;
                contract.reward_contract_address = reward_contract_address;
        ...
        fn handle_bid(
            ...
            if let Some(winning) = self.winning {
                let winning_balance = self.balances.get(&winning).unwrap_or(0u128);
            ...
            if let Some(old_balance) = self.balances.get(&bidder) {
            ...
            self.winning_data
                .insert(offset.try_into().unwrap(), Some((bidder, bid)));
            self.env().emit_event(Bid { from: bidder, bid });
            Ok(())
        }
        ...
        fn pay_back(&mut self, reward: fn(&Self, to: AccountId) -> (), to: AccountId) {
            ...
            if let Some(bal) = self.balances.get(&to) {
        ...
        fn blow_candle(&self, seed: &[u8]) -> Option<(AccountId, Balance)> {
            ...
            if ending_period_last_block <= known_since {
                ...
                let u_offset: usize = offset.try_into().unwrap();
                // Detect winning slot.
        ...
        pub fn get_winning(&self) -> Option<(AccountId, Balance)> {
            if let Some(winning) = self.winning {
                let bid = self.balances.get(&winning).unwrap_or(0u128);
                Some((winning, bid))
            } else {
                None
            }
